...
namespace OrderWebAPITest.DomainValidationTests
{
    public class PersonTests
    {
        ...
        {
            // Arrange and act
            Person person = new Person("Vuk", "Đedović", "Jurija Gagarina14G, Beograd", "0645233699", "silaris@gmail.com", 1);

            // Assert
            Assert.NotNull(person);
            Assert.NotEmpty(person.FirstName);
            Assert.NotEmpty(person.LastName);
            Assert.NotEmpty(person.Adress);
        ...
        public void ArgumentException_Thrown_When_FirstName_Length_Is_More_Than_15_Characters(string firstName, string lastName,
            string adress, string phoneNumber, string email, int id)
        {
            // Assert
            Assert.Throws<ArgumentException>(() => new Person(firstName, lastName, adress, phoneNumber, email, id));
        ...
        public void ArgumentException_Thrown_When_LastName_Length_Is_More_Than_15_Characters(string firstName, string lastName,
            string adress, string phoneNumber, string email, int id)
        {
            // Assert
            Assert.Throws<ArgumentException>(() => new Person(firstName, lastName, adress, phoneNumber, email, id));
